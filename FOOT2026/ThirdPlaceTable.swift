...
//
// Slot keys → match:
// "A/B/C/D/F"  → M74   "C/D/F/G/H"  → M77   "C/E/F/H/I"  → M79   "E/H/I/J/K"  → M80
// "B/E/F/I/J"  → M81   "A/E/H/I/J"  → M82   "E/F/G/I/J"  → M85   "D/E/I/J/L"  → M87
...
extension MatchStore {
    static let thirdPlaceTable: [String: [String: String]] = {
        ...
        // Each row: "<8-group key> <8 assigned-group letters>" (column order = `slots`).
        // Stored as one string literal so the optimizer never builds a giant
        // array initializer (avoids a SIL ColdBlockInfo blow-up at -O).
        let rows = """
        EFGHIJKL EJIFHGLK
        DFGHIJKL HGIDJFLK
        DEGHIJKL EJIDHGLK
        ...
        ABCDEFGH HGBCAFDE
        """
        let raw: [(String, [String])] = rows.split(separator: "\n").map { line in
            let p = line.split(separator: " ")
            return (String(p[0]), p[1].map { String($0) })
        }
        var result: [String: [String: String]] = [:]
        for (key, vals) in raw {
            var dict: [String: String] = [:]
            for (i, slot) in slots.enumerated() { dict[slot] = vals[i] }
            result[key] = dict
        }
        return result
    }()
